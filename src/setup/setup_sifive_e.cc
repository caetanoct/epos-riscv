...
void _entry() // machine mode
{
    ...
    CPU::sp(Memory_Map::BOOT_STACK - Traits<Machine>::STACK_SIZE * (CPU::id() + 1)); // set this hart stack
    if(Traits<System>::multitask) {
...
void _setup()
{
    ...
    if(CPU::id() != 0) {
        CLINT::stvec(CLINT::DIRECT, _int_wfi);        // setup a supervisor mode interrupt handler
